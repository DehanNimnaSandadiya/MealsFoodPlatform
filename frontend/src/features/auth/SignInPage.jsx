...
export function SignInPage() {
  ...
  const from = location.state?.from?.pathname || "/role-redirect";
  const base = typeof window !== "undefined" ? window.location.origin : "";
  const afterSignInUrl = base ? `${base}${from.startsWith("/") ? from : `/${from}`}` : from;
    ...
    <div className="grid min-h-[calc(100vh-8rem)] md:grid-cols-2">
      ...
      <div className="flex flex-col justify-center bg-white px-6 py-8 md:px-12">
        <div className="mx-auto w-full max-w-sm">
          <h1 className="font-display text-2xl font-bold text-black">Welcome back</h1>
          <p className="mt-1 text-sm text-black/60">Sign in to continue to meals</p>
          ...
          <p className="mt-6 text-center text-sm text-black/60">
            ...
            <Link to="/sign-up" className="font-medium hover:underline" style={{ color: BRAND }}>
              Sign up
            </Link>
          </p>
          <p className="mt-4 text-center text-sm text-black/60">
            <Link to="/apply/seller" className="font-medium hover:underline" style={{ color: BRAND }}>
              Apply to be a Home Based Seller
            </Link>
            <br />
            <Link to="/apply/rider" className="mt-1 inline-block font-medium hover:underline" style={{ color: BRAND }}>
              Apply to be a Rider
            </Link>
          </p>
        </div>
